...
def save():
    ...
    show = ("SELECT * FROM appointment where P_ID='{}' and Date='{}'").format(en1,en2)
    cursor.execute(show)
    ...
    if(len(data1)==0):
        ...
        show=("SELECT * from appointment WHERE D_Name = '{}' and Date='{}'").format(en3,en2)
        cursor.execute(show)
        data=cursor.fetchall()
            
        ano = len(data)+1
        
        insert = ("insert into appointment values('{}','{}','{}',{})").format(en1,en3,en2,ano)
